...
- `--bot-gpu-emulation` (ENT Tier2, default true): Controls GPU rendering backend selection on Linux. Automatically detects and prefers system GPU/GL drivers when available for optimal performance. Set `--bot-gpu-emulation=false` to use your own GPU or GL driver directly. Guide: [Linux GPU Backend Selection](docs/guides/deployment/LINUX_GPU_BACKEND.md)
